...
> **Configuration Stability (v0.3.1):** Empty YAML keys (such as `disabled:` or `severity_overrides:` with only comments or empty values) are safely normalized to empty lists and dictionaries. The canonical configuration generated by `envguard init` uses explicit empty collections (`exclude: []`, `disabled: []`, `severity_overrides: {}`). Invalid configuration types consistently trigger clean diagnostic messages and exit code `2` without silent fallbacks.
